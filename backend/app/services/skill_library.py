        ...
        "modules": [
            ...
            },
            {
                ...
                "content": (
                    ...
                    "- 乘法公式\n"
                    "  - 伤害=1-守方防御/(守方防御+K)\n"
                    "    - K 值一般结构为 K(L) = base_K + K_step(L)\n"
                    "    - base_K 为常数，K_step 为随等级增加的步进值\n"
                    "    - 膨胀速率低（max ATK/L1 ATK≤10）：K_step 可以为 0（纯常数 K）\n"
                    "    - 膨胀速率高（max ATK/L1 ATK>10）：K_step 必须 ≥0 且随等级增长\n"
                    "    - 注意不要删除原 def_K 常量，它作为 base_K 仍然有效\n"
                    "  - 乘法公式下，默认攻击防御按照 1:1 投放\n"
                    "  - HP/ATK 比值是唯一验收标准：全等级 ratio ∈ [5, 100] 且单调递增\n"
                    "    - 比值差异本质上就是 K 值导致的结果，调节 K 即可控制比值\n"
                    "    - 在 HP 反推步骤中必须根据 HP/ATK 比值趋势反向校验 K 值是否合理"
                ),
            },
            {
                "module_key": "attack_speed",
                "title": "攻击速度",
                "required": True,
                "enabled": True,
                "content": (
                    "- 当游戏为即时制战斗时，首先要规定一个全局基本攻击间隔（需注册为常量），若用户无约定，默认为2s/Hit（值=2）\n"
                    "- 攻击速度作为属性时，含义是攻击速度增幅，攻击速度=0时，单位以默认攻击速度行动\n"
                    "  - 攻击速度属性一般而言按百分比处理\n"
                    "- 基础攻击间隔修正\n"
                    "  - 这是一个对于具体单位（任何具有普攻能力的单位）的必选属性\n"
                    "  - 本值是一个小数，单位的实际基础攻击间隔 = 全局基本攻击间隔 + 基础攻击间隔修正，"
                    "这个值跟随单位一辈子，一般来说不可改变（若用户要求改变则也允许）\n"
                    "  - 单主角时，主角的攻速修正可注册到常量表，怪物或其他单位的攻速修正注册到怪物\\玩法表\n"
                    "  - 若有职业系统\\武器系统或多主角，则攻速修正注册到职业\\武器\\角色等这些具体配置表\n"
                    "- 全局最低攻击间隔\n"
                    "  - 这是一个全局参数（需注册为常量），当通过攻击间隔修正和攻击速度修正后，最终攻击间隔若短于本值，则改为本值，若无约定，设为0.15\n"
                    "- 攻击速度算法\n"
                    "  - 若单位身上有多个攻击速度属性（如装备属性，BUFF\\DEBUFF影响等），先把所有攻击速度做加法，得到一个最终攻击速度\n"
                    "  - 若最终攻击速度>0，为提高攻击速度，最终攻击间隔 = 单位攻击间隔 / (1 + 攻击速度)\n"
                    "  - 若最终攻击速度<0，为减少攻击速度，最终攻击间隔 = 单位攻击间隔 * (1 - 攻击速度)"
                ),
            },
            {
                "module_key": "hp",
                ...
                "content": (
                    ...
                    "    - 有效输出量就是HP的最终数值\n"
                    "- ★ K 值校验（乘法防御公式必须执行）：反推 HP 之前必须检查并调整 K 值\n"
                    "  - K 值一般结构为 K(L) = base_K + K_step(L)\n"
                    "  - base_K 为原 def_K 常量，K_step 为随等级增加的步进常量\n"
                    "  - 膨胀速率低时 K_step 可为 0（纯常数 K），膨胀速率高时 K_step 必须随等级增长\n"
                    "- ★ HP/ATK 比值校验（验收关卡，必须执行）：\n"
                    "  - 全等级 ratio(L) = hp(L) / atk(L) 必须在 [5, 100] 范围内\n"
                    "  - ratio(L) 必须随等级单调递增（或至少不递减），禁止高等级比值降低\n"
                    "  - HP/ATK 比值的差异本质上就是 K 值导致的结果，调节 K 即可控制比值\n"
                    "  - 若不满足：优先调 K（改 base_K 或 K_step），次选微调 expected_survive_seconds\n"
                    "  - 调整后必须重新计算 hp 并再次校验，直到全部条件满足"
                ),
            },
            {
                ...
                "content": (
                    ...
                    "  - 此类效果需要注意最终实战不应超过30%，但不是指投放不能超过30%，"
                    "而是说需要始终投放足够的命中\\设计怪物时需要让怪物具有更高的命中来平衡，使玩家实战闪避不超标；"
                    "单从投放角度而言，闪避的投放量级应当跟暴击率这类属性相当（若没有对抗属性则按照上限30%去衡量即可）\n"
        ...
        "modules": [
            {
                ...
                "content": (
                    ...
                    "- 独立战斗单位也需要在总表中分配属性，但是这个分配值不占用总占比，但用于说明跟总属性的比例关系\n"
                    "  - 例如装备10%意味着装备要发放这10%属性，伙伴_基本=10%意味着的是伙伴基础属性是总属性的10%，"
                    "但并不会发放10%属性给主角或者其他独立战斗单位\n"
                    "- 对于独立战斗单位，还要有对于其更加细致的属性再分，"
                    "例如宠物总体生命=80%模型生命，其中升级带来的是40%，宠物装备带来的是40%（仅举例）\n"
                    "- 独立战斗单位的属性分配也需要是最低属性粒度的\n"
                    "- 独立战斗单位系统仍然可能给主角提供属性\n"
                    "  - 例如很多设计中独立战斗单位的属性有10%会转换为主角属性，或者有的设计中独立战斗单位的图鉴系统会提供属性给主角\n"
                    "  - 这些时候需要更准确地做拆分，如伙伴_图鉴，伙伴_属性转化等等\n"
                    "  - 这一部分属性归谁（属于独立战斗单位还是属于主角）需要明确，并据此决定其到底是否占用总占比\n"
            ...
            },
            {
                ...
                "enabled": True,
                "content": (
                    "- 此处必须按照最小系统粒度分配系统属性，例如装备有基础属性和强化属性，"
                    "必须按照装备_基础，装备_强化来分配，不能仅分配装备或者分配装备_总\n"
            ...
            },
            {
                ...
                "content": (
                    ...
                    "- 玩法落地时也可以根据实际情况再次调整玩法分配数值，或者用户要求时你也应当调整玩法分配的数值或者基础属性框架\n"
                    "  - 调整后必须抽查检验每个玩法，确认没有产生数值问题\n"

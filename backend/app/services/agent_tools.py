...
TOOLS_OPENAI: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "标注表的类型分类：\n"
                "★ `config`：配置表（所有列的值均来自策划设计决策，无论值是手动填入还是通过公式计算——"
                "只要公式表达的是策划意图而非纯数学推导，即为配置列。典型配置列：属性初值、养成曲线查表值、掉落概率等）\n"
                "★ `compute`：计算表（所有列均为纯数学推导，如 @base_atk * (1 + @atk_amp)，不包含策划设计决策）\n"
                "★ `mixed`：混合表（部分列为配置值，部分列为纯推导计算）→ 必须传 column_kinds\n"
                "★ 混合表时 column_kinds 必填：JSON 映射每列的角色\n"
                "  'config'＝配置列（含公式填充的配置量）、'compute'＝纯推导计算列、'skip'＝不参与计算（如 id 列/备注列等）\n"
                "★ 前端按类型筛选表列表，计算列显示浅灰底色、配置列显示浅棕底色，务必在创建完成后首次标注"
            ),
            "parameters": {
                "type": "object",
                "properties": {

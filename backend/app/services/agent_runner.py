...
def _base_common_system(mode_norm: str) -> str:
    ...
    return "\n".join(
        [
            ...
            "  · config（配置表）：所有列均承载策划设计决策。注意：公式填充的配置量（如查表取值、常量引用、"
            "IF 条件分配）语义上仍属于配置列，不算计算列。计算列的判定标准是「纯数学推导不包含设计选择」。\n"
            "  · compute（计算表）：全部列为纯数学推导（如 @base * 系数），不包含任何设计决策。\n"
            "  · mixed（混合表）：部分列为配置、部分列为纯计算，必须传 column_kinds 逐列标注。\n"
            "  · 混合表每列标注：'config'=配置列（含公式填充的配置量）、'compute'=纯推导计算列、'skip'=不参与计算。\n"
            "  · 客户端显示规则：计算列浅灰底色、配置列浅棕底色、未标注列白色。",
...
_EXECUTE_SYSTEM_TAIL = (
    ...
    "design_intent 字段记录**设计决策与边界**（为什么是这个值 / 可调范围 / 待验证假设），强烈建议填写；\n"
    "  与 brief 明确分工：brief = 是什么（概念定义），design_intent = 为什么/怎么调（设计意图）。\n"
    "  ✓ '与 player_crit_rate 保持一致，后续可上调至 0.15' / '暂定值，待战斗节奏验证' / '上限为 0.8，超额无效'\n"
    "  ✗ 留空（丢失调参依据） / 与 brief 重复（如'基础暴击率'）\n\n"

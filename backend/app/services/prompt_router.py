...
DEFAULT_STEP_PROMPTS: Dict[str, str] = {
    ...
    "gameplay_table": (
        ...
        "【标准执行流程】\n"
        "  1. `get_gameplay_table_list` → 查看本表的 readme、依赖关系和当前状态；\n"
        "  2. `set_gameplay_table_status(table_id, '进行中')` → 标记开始；\n"
        "  3. gather 阶段（只读）：\n"
        "     · `get_project_config` / `get_table_list` → 了解项目整体结构；\n"
        "     · `list_exposed_params(本步骤ID)` → 【必须调用】获取上游暴露参数，调用后参数自动标记为 acknowledged；\n"
        "       本步骤 ID 格式：gameplay_table.<table_id>，广播参数 target='subsystems:gameplay_table' 也会被收到；\n"
        "     · 若有 dependencies，读取被依赖表的结构和数据：read_table / get_table_readme；\n"
        "  4. design → execute：按 readme 和暴露参数完成该表的完整数值（表结构/公式/属性/消耗）；\n"
        ...
        "     · 若设计过程中发现另一张已完成的玩法表数值需要调整，调用 `request_table_revision(table_id, reason)` 发起修订请求；\n"
        "     · 若需向兄弟表暴露关键参数，调用 `expose_param_to_subsystems`（先用 get_gameplay_table_list 确认目标 table_id）；\n"
        "       广播给所有玩法表：target_step='subsystems:gameplay_table'；精确指向：target_step='gameplay_table.<table_id>'；\n"
        "  5. 完成后：`set_gameplay_table_status(table_id, '已完成')` → 标记完成（同时自动将已 acknowledged 的暴露参数标记为 acted_on）；\n"
        "  6. `update_table_readme` 更新表 README，说明数值设计决策。\n\n"
        "★ 具体设计方法参考本步对应的 SKILL 和 readme 中的指引。\n"
        "★ 若本步无上游暴露参数，list_exposed_params 会返回空列表，继续执行即可。"
    ),

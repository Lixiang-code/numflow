...
DEFAULT_STEP_PROMPTS: Dict[str, str] = {
    ...
    "hp_formula_derivation": (
        ...
        "先 `get_project_config` → 读取 stat_keys（确认攻击属性列名/防御属性列名）和防御公式类型。\n"
        "\n"
        "★★ 关键前置步骤：K 值校验与调整（乘法防御公式必须执行，减法公式跳过）★★\n"
        "  K 值决定了 HP/ATK 比值——K 值越大减伤越低、HP/ATK 比值越小。\n"
        "  步骤 3 中设定的 K 值只是初步估计（常数），本步必须基于实际 ATK 数据重新校验：\n"
        "  1. `const_list` 读取当前 K 值常量（一般为 <def_attr>_K），保留作为基础 K 值；\n"
        "  2. 判断 K 值是否需要加上等级变量（切勿默认只用常数）：\n"
        "     - 膨胀速率低（max ATK / L1 ATK ≤ 10 倍）：只用常数 K 即可；\n"
        "     - 膨胀速率高（max ATK / L1 ATK > 10 倍）：K 应当 = base_K + K_step(L)\n"
        "       其中 base_K 为原常数值，K_step(L) 为随等级增加的部分\n"
        "       （`const_register('<def_attr>_K_step', 步进值, tags=['combat'])，\n"
        "        等级步进 K_step 可按指数增长率或线性增长设计）；\n"
        "  3. 更新 num_base_framework 的 <def_attr>_reduction 列公式，\n"
        "     def_ratio(L) = def_attr(L) / (def_attr(L) + base_K + K_step(L))；\n"
        "  4. 注意：不要删除原 def_K 常量，它作为 base_K 仍然有效。\n"
        "\n"
        "核心推导路径：\n"
        "  hp(L) = atk_ref(L) × net_dmg_ratio(L) × expected_survive_seconds(L) × attacks_per_second\n"
        "  乘法模型：net_dmg_ratio(L) = 1 - def_attr(L) / (def_attr(L) + base_K + K_step(L))\n"
        "  减法模型：net_dmg = atk - def（K 值不适用，跳过 K 值校验步骤）\n"
        ...
        "  5. `update_table_readme`：写出公式、战斗节奏假设、K 值校验过程、level1/mid/max 的 HP 合理性校验。\n"
        "\n"
        "★★ HP/ATK 比值校验（必须执行，验收关卡）★★\n"
        "  反推完成后求全等级 ratio(L) = hp(L) / atk_ref(L)，必须同时满足：\n"
        "  a. 全等级 5 ≤ ratio(L) ≤ 100；\n"
        "  b. ratio(L) 随等级单调递增（或至少不递减）；\n"
        "  c. 若任一条件不满足：优先通过调整 K 值（改 base_K 或调 K_step）来修正，\n"
        "     次选微调 expected_survive_seconds；\n"
        "     调整后重新计算 hp 列并再次校验，直到 a/b 条件全部满足；\n"
        "  d. 将 L1、Lmid、Lmax 三点的 ratio 值及最终 K 值结构写入 README。\n"
        "\n"
        "★ review 阶段：对比全等级 hp/atk 比值趋势。\n"
    ...
    "gameplay_planning": (
        ...
        "     · display_name：中文任务名\n"
        "     · readme：至少 50 字，说明任务目标、关键产物、依赖关系\n"
        "  7. 最后调用 `get_gameplay_table_list` 确认所有任务均已注册且状态为「未开始」。\n"
        "★ 本步严禁调用 create_table / write_cells / setup_level_table 等建表写数工具。\n"
        "★ 每个在 game_systems 中启用的玩法系统至少注册 1 个任务；\n"
        "  根据系统复杂度和 get_default_system_rules 的细则决定是否拆分子任务。\n"
        "验收：get_gameplay_table_list 返回非空列表，所有任务状态均为「未开始」。"
    ),
    "gameplay_table": (
        _NAMING_HEADER
        + "【步骤 8+/7+N 任务执行（动态步骤，任务池模式）】\n"
        "目标：从任务池中选择一个任务执行，完成后标记。任务池是动态的，可随时新增或修订。\n"
        "每次只做一个任务；系统会自动检查队列，若仍有未完成任务则发起下一轮 agent。\n\n"
        "【步骤〇：前置检查（必读）】\n"
        "  执行任务过程中，若发现缺失的子任务（如缺少某张对照表、某子系统未规划），\n"
        ...
        "     · 若发现已完成任务数值需调整：`request_table_revision(table_id, reason)` 入队等待后续处理；\n"
        "     · 若需向兄弟任务暴露约束参数：`expose_param_to_subsystems`；\n"
        "     · 【强制规则】你需要的资源或属性必须已经在资源总表（num_resource_framework）、\n"
        "       属性总表（num_base_framework）、资源分配表（gameplay_res_alloc）、\n"
        "       属性分配表（gameplay_attr_alloc）中被注册或提及。\n"
        "       若发现未纳入管理的资源或属性：\n"
        "       a. 先到对应总表中补全（num_resource_framework 补资源、num_base_framework 补属性）；\n"
        "       b. 再到分配表中追加该资源/属性的分配比例（gameplay_res_alloc / gameplay_attr_alloc）；\n"
        "       c. 完成后才能在本任务中引用该资源/属性。\n"
        "       绝对禁止使用任何未出现在总表及分配表中的资源或属性。\n"
        "  4. 若认为当前工作需要的必要前置工作没有完成或信息不足，`request_table_revision(table_id, reason)` 入队等待处理；若当前无法推进，结束本次任务并将本任务标记为「未开始」方便后续回归；\n"
        ...
        "★ 每次 agent 只完成一个任务，专注完成好一个，不要尝试处理多个。"
    ),
}

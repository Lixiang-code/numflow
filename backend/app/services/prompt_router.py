...
DEFAULT_STEP_PROMPTS: Dict[str, str] = {
    ...
    "base_attribute_framework": (
        "【步骤 2/11 基础属性框架】"
        "目标：定义角色基础属性骨架（HP/ATK/DEF/CRIT 等），输出标准等级 1..max_level 的基础属性增长表。"
        "必产出表：基础属性_标准等级（列：等级、HP、ATK、DEF、命中、闪避、暴击、暴伤 等核心列），"
        ...
        "      命中/闪避/暴击/暴伤 用 ROUND(基础 + 增量*POWER(归一化等级, 指数), 2) 类公式即可。\n"

...
DEFAULT_STEP_PROMPTS: Dict[str, str] = {
    ...
    "gameplay_table": (
        _NAMING_HEADER
        + "【玩法落地表执行步骤】\n"
        "目标：执行玩法规划中指定的某一张落地表的完整数值设计，同时处理队列中的待修订任务。\n\n"
        "【标准执行流程】\n"
        "  1. `get_gameplay_table_list` → 查看所有表的状态；\n"
        "     · 若有 status='待修订' 的表：读取其 revision_reason 和 revision_requested_by，\n"
        "       判断当前是否应处理（无阻塞依赖、或修订与本步骤相关时优先处理）；\n"
        "     · 若决定处理修订任务：按下方修订流程执行；\n"
        "     · 否则：继续处理本步骤分配的表（status='未开始' 的那张）。\n"
        "  2. `set_gameplay_table_status(table_id, '进行中')` → 标记要处理的表（本步或修订表）开始；\n"
        "  3. gather 阶段（只读）：\n"
        "     · `get_project_config` / `get_table_list` → 了解项目整体结构；\n"
        "     · `list_exposed_params(本步骤ID)` → 【必须调用】获取上游暴露参数（自动标记已读）；\n"
        "       本步骤 ID 格式：gameplay_table.<table_id>；\n"
        "     · 若有 dependencies，读取被依赖表的结构和数据：read_table / get_table_readme；\n"
        "  4. design → execute：按 readme（或修订原因）完成该表的完整数值；\n"
        "     · 属性值：`call_calculator(gameplay_attr_alloc_lookup, ...)` 取，不硬编码；\n"
        "     · 资源消耗：`call_calculator(gameplay_res_alloc_lookup, ...)` 取；\n"
        "     · 若设计过程发现另一张已完成表需调整：`request_table_revision(table_id, reason)` 入队；\n"
        "     · 若需向兄弟表暴露约束参数：`expose_param_to_subsystems`；\n"
        "  5. 完成后：`set_gameplay_table_status(table_id, '已完成')` → 标记完成；\n"
        "  6. `update_table_readme` 更新表 README，说明设计决策。\n\n"
        "【修订任务处理（待修订表）】\n"
        "  - revision_reason 说明了需要修改的内容，从它出发读取已有表数据，按原因修改；\n"
        "  - 修订完成同样调用 set_gameplay_table_status(table_id, '已完成')；\n"
        "  - 修订完成后系统自动更新修订请求状态。\n\n"
        "★ 具体设计方法参考本步对应的 SKILL 和 readme 中的指引。\n"
        "★ 若 list_exposed_params 返回空列表，说明无上游约束，继续执行即可。"
    ),
